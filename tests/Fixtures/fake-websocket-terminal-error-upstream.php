...
if ($port <= 0 || $captureFile === '') {
    fwrite(STDERR, "usage: fake-websocket-terminal-error-upstream.php <port> <capture-file>\n");
    exit(2);
}
...
$server->on('message', static function (Server $server, Frame $frame) use ($captureFile): void {
    ...
    $server->push($frame->fd, '{"type":"error","error":{"code":"server_error","message":"boom"}}');
    $server->disconnect($frame->fd, \SWOOLE_WEBSOCKET_CLOSE_NORMAL, 'terminal error');
});
$server->start();
