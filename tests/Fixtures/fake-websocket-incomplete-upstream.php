...
if ($port <= 0 || $captureFile === '') {
    fwrite(STDERR, "usage: fake-websocket-incomplete-upstream.php <port> <capture-file>\n");
    exit(2);
}
...
$server->on('message', static function (Server $server, Frame $frame) use ($captureFile): void {
    ...
    $server->push($frame->fd, '{"type":"response.output_text.delta","delta":"Hello"}');
    usleep(20_000);
    $server->disconnect($frame->fd, \SWOOLE_WEBSOCKET_CLOSE_NORMAL, 'upstream closed early');
});
$server->start();
