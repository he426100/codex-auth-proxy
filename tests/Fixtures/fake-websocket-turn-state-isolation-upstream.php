...
if ($port <= 0 || $captureFile === '') {
    fwrite(STDERR, "usage: fake-websocket-turn-state-isolation-upstream.php <port> <capture-file>\n");
    exit(2);
}
...
$server->on('handshake', static function (Request $request, Response $response) use ($captureFile, &$connectionCount, &$connectionIndexes): bool {
    ...
    $turnMetadata = $request->header['x-codex-turn-metadata'] ?? null;
    file_put_contents($captureFile, json_encode([
        ...
        'turn_metadata_header' => is_string($turnMetadata) && $turnMetadata !== '' ? $turnMetadata : null,
    ...
    if ($connectionCount === 1) {
        $headers['x-codex-turn-state'] = 'ts-first-turn';
    }
...
$server->on('message', static function (Server $server, Frame $frame) use ($captureFile, &$connectionIndexes): void {
    ...
    $responseId = $connectionIndex === 1 ? 'resp_ws_first_turn' : 'resp_ws_second_turn';
    ...
    if ($connectionIndex === 1) {
        Timer::after(20, static function () use ($server, $fd): void {
            $server->disconnect($fd, 1000, 'first turn complete');
        });
    }
});
