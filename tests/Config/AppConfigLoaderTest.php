...
final class AppConfigLoaderTest extends TestCase
{
    public function testLoadUsesCommittedDefaults(): void
    {
        ...
        try {
            ...
            self::assertSame('codex-auth-proxy/0.1.0', $config->codexUserAgent);
            self::assertSame('', $config->codexBetaFeatures);
            self::assertSame('codex-auth-proxy', $config->codexOriginator);
